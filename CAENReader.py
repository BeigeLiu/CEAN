...
class RawTrigger:
    ...
    def loadfromdict(self,Dict):
        ...
        self.filePos= Dict['filePos']  
        self.triggerTimeTag=Dict['triggerTimeTag'] 
        self.triggerTime=Dict['triggerTime']    
        self.signaltype=Dict['type']          
        self.peakindex=Dict['peak_index']     
        self.peakwidth=Dict['peak_width']     
        self.charge=Dict['charge']         
        self.integrate_range=Dict['integrate_range']
        return
    def todict(self):
        Dict = {}
        Dict['traces']   = self.traces
        Dict['filePos']  = self.filePos
        Dict['triggerTimeTag'] = self.triggerTimeTag
        Dict['triggerTime']    = self.triggerTime
        Dict['type']           = self.signaltype
        Dict['peak_index']     = self.peakindex
        Dict['peak_width']     = self.peakwidth
        Dict['charge']         = self.charge
        Dict['integrate_range']= self.integrate_range
        return Dict
    def display(self, trName=None):
        ...
        if trName is None:
            ...
            for trace in sorted(self.traces.items()):
                plt.plot(trace[1][5:], label=trace[0],drawstyle = 'steps-mid')
                plt.vlines(self.integrate_range[i],
                           np.min(trace[1][5:]),
                           np.max(trace[1][5:]),
            ...
            if isinstance(trName, str):
                plt.plot(self.traces[trName][5:], label=trName,drawstyle = 'steps-mid')
            elif isinstance(trName, list):
                for t in trName:
                    plt.plot(self.traces[t][5:], label=t,drawstyle = 'steps-mid')
        ax.legend(loc=0)
